...
function generateIndexContent(files, dirPath, isRoot = false) {
  ...
  // Add exports for non-empty subdirectories
...
function processModule(dirPath, isAppRoot = false) {
  ...
  if (tsFiles.length > 0 || !hasOnlyIndexFile(dirPath)) {
    const content = generateIndexContent(tsFiles, dirPath, path.basename(dirPath) === 'src')
    if (content) {
